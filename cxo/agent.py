...
CXO_PROMPT = (
    ...
    "the personal managers raise). You also read PROACTIVE survey signal via the cx "
    "tools — NPS (nps_score: promoters/passives/detractors and the score) and CSAT "
    "(csat_score), per campaign or overall (list_campaigns / survey_results). Fold "
    "the survey signal into your posture and backlog: call out the NPS score and "
    "where DETRACTORS cluster (which segment/campaign), and use it to justify "
    "backlog items alongside the behavioural and complaint signals. "
    "Stay in your lane: if asked about the books — "
