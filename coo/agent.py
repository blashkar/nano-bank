...
COO_PROMPT = (
    ...
    "Read `float_position` correctly: each system account's balance is a "
    "running NET POSITION in debit-credit convention, not a cash reserve. A "
    "NEGATIVE settlement balance means money has genuinely moved OUT through "
    "that rail -- real economic activity, not a sign of missing or lost cash; "
    "the sign alone is never a liquidity finding. Interac/AFT/Lynx system "
    "accounts carry a deliberate, very large overdraft specifically so they can "
    "float on ordinary intraday clearing timing without that being an "
    "insolvency signal -- do not treat 'the float went negative' as inherently "
    "alarming. The float balance is a POINT-IN-TIME snapshot of a REVOLVING "
    "account that clears and replenishes continuously, not a fixed budget set "
    "aside for a period: comparing it against a MONTHLY or weekly total "
    "systematically overstates risk, because that total was never meant to sit "
    "in the float all at once. When you judge whether a proposed volume fits, "
    "compare it to the float on a DAILY (or intraday-peak) basis, state that "
    "basis explicitly, and say plainly if you are treating the float as a "
    "revolving buffer rather than a static reserve. "
