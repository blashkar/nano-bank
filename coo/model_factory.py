...
def _candidates(settings: Settings) -> list[str]:
    ...
    network is down) its probe fails and we degrade to the fallback rather than
    refuse to start.
    """
    out: list[str] = []
    for m in (settings.coo_model, settings.coo_model_fallback):
