...
# (open issue / dormant), +1 positive (active, no issue). customer_sentiment()
# only ever produces one of those two signs — there's no third, weaker signal
# to distinguish a neutral respondent from a positive one yet — so there is no
# 0 (mixed) bucket here either; add one back if a real neutral signal shows up.
_NPS_BUCKETS = {
...
def customer_sentiment(db, targets: list[str], window_days: int) -> dict:
    """-1 for customers with an open issue or dormant, +1 for active-no-issue."""
...
def create_campaign(db, instrument: str, segment: str, question: str,
                    seed: int = 7, window_days: int = 30,
                    source: str = "demo_seed") -> dict:
    if instrument not in ("nps", "csat"):
        raise ValueError(f"unknown instrument: {instrument}")
    targets = db.resolve_segment(segment, window_days)
    campaign_id = db.insert_campaign(instrument, segment, question, source=source)
